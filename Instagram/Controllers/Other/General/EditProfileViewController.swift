...
struct EditProfileFormModel{
    // every form model is going to have two properties in it
    let label: String
...
final class EditProfileViewController: UIViewController, UITableViewDataSource{
    ...
    private var models = [[EditProfileFormModel]]() //since we are going to have two sections in here
    ...
    private func configureModels(){
        ...
        for label in section1Labels {
            let model = EditProfileFormModel(label: label, placeholder: "Enter \(label)...", value: nil) // we should query from the database the current value and then use that,
